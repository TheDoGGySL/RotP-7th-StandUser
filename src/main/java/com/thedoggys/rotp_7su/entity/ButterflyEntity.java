...
public class ButterflyEntity extends ItemProjectileEntity {
    private boolean timeStop = false;
    private Vector3d timeStopHitMotion;
    private int tsFlightTicks = 0;
    private TexVariant knifeTexVariant = TexVariant.KNIFE;

    public ButterflyEntity(World world, LivingEntity shooter) {
            super(InitEntities.KNIFE.get(), shooter, world);
    }

    public ButterflyEntity(World world, double x, double y, double z) {
        super(InitEntities.KNIFE.get(), x, y, z, world);
    }
...
//    public void setSecondsOnFire(int seconds) {}
        ...
        if (!level.isClientSide()) {
            entity.getCapability(LivingUtilCapProvider.CAPABILITY).ifPresent(cap -> {
                SidedItemTrackerMap.getSidedTrackers(level).values().stream()
                        ...
                        .forEach(tracker -> {
                            tracker.setAtEntity(entity.getId(), level, TrackerItemStack.KnownItemState.STUCK_KNIFE);
                        });
    ...
    public static enum TexVariant {
        KNIFE(new ResourceLocation(AddonMain.MOD_ID, "textures/entity/projectiles/knife.png"));
