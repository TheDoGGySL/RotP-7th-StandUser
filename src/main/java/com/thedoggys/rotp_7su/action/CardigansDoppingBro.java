...
public class CardigansDoppingBro extends StandEntityAction {
    boolean manifest = true;
    public static final StandPose INJECT_TARGET = new StandPose("inject");

    public CardigansDoppingBro(Builder builder) {
        super(builder);
    }
        ...
        if (!world.isClientSide) {
            if (task.getTarget().getType() == ActionTarget.TargetType.ENTITY) {
                ...
                if (target instanceof LivingEntity) {
                    ((LivingEntity) target).addEffect(new EffectInstance(InitEffects.DOPING_EFFECT.get(), 600, 0, false, false));
                }
            }
        }
