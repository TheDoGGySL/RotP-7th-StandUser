...
public class MiraclesSizzle extends StandEntityAction {

    public MiraclesSizzle(Builder builder) {
        super(builder);
    }
        ...
        if (!world.isClientSide) {
            if (task.getTarget().getType() == ActionTarget.TargetType.ENTITY) {
                ...
                if (target instanceof LivingEntity) {
                    ((LivingEntity) target).addEffect(new EffectInstance(InitEffects.SIZZLE_EFFECT.get(), 300, 0, false, false));
                }
            }
        }
